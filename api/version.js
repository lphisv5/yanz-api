...
async function fetchIosVersion() {
  ...
  const scriptRegex = /<script type="application/ld+json"[^>]*>([sS]*?)</script>/gi;
  let match;
  ...
  const versionPattern = /Version[sS]{0,300}?(2.d{3}.d{3,4})/i;
  ...
  const whatsNewPattern = /"whatsNew"[sS]{0,500}?"version"[sS]{0,100}?"string"[sS]{0,100}?"([d]+.[d]+.[d]+)"/i;
  ...
  const appStorePattern = /"versionDisplay"[sS]{0,100}?"([d]+.[d]+.[d]+)"/i;
...
export default async function handler(req, res) {
  ...
  if (!['android', 'ios', 'windows'].includes(platform)) {
    return res.status(400).json({ error: 'Invalid platform. Use android, ios or windows' });
  }

  try {
    let version, clientVersionUpload = null;
    ...
    const responseData = { version, platform };
